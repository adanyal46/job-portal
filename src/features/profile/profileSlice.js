...
export const getProfile = createAsyncThunk("profile/getProfile", async (_, { rejectWithValue }) => {
  try {
    ...
    if (USER_ROLE === "JOB_SEEKER") {
      ...
      apiRoute = await recruiterProfileApi();
    }
...
export const profileUpdate = createAsyncThunk("profile/updateInfo", async (formData, { getState, rejectWithValue }) => {
  try {
...
export const profileEducation = createAsyncThunk("profile/education", async (formData, { getState, rejectWithValue }) => {
  try {
...
export const profileLocation = createAsyncThunk("profile/location", async (formData, { getState, rejectWithValue }) => {
  try {
...
export const profileDocument = createAsyncThunk("profile/documents", async (formData, { getState, rejectWithValue }) => {
  try {
...
export const profileEmploymentHistory = createAsyncThunk("profile/employment", async (formData, { getState, rejectWithValue }) => {
  try {
    const currentUser = getState().profile.user;
    ...
    if (employmentId) {
      ...
      const updatedEmploymentHistory = currentUser?.EmpolymentHistory.map((emp) =>
        emp.id === employmentId
          ? {
              ...emp,
              ...updatedData.data,
      ...
      const updatedEmploymentHistory = [...currentEmploymentHistory, newEmploymentEntry];
...
export const profileCertificate = createAsyncThunk("profile/certificate", async (formData, { getState, rejectWithValue }) => {
  try {
    const currentUser = getState().profile.user;
    ...
    if (certificateId) {
      ...
      const updateCertificateHistory = [...currentCertificate, newCertificate];
...
export const deleteEducation = createAsyncThunk("profile/education/delete", async (formData, { getState, rejectWithValue }) => {
  try {
    const response = await deleteEducationApi(formData);

    const currentUser = getState().profile.user;
    const updatedEducation = currentUser.Education.filter((edu) => edu.id !== formData?.educationId);
...
export const deleteCertificate = createAsyncThunk("profile/certificate/delete", async (formData, { getState, rejectWithValue }) => {
  try {
    const response = await deleteCertificateApi(formData);
    const deleteId = formData.certificateId;
    const currentUser = getState().profile.user;
    const updatedCertificate = currentUser.Certificate.filter((cer) => cer.id !== deleteId);
    return {
      success: response.success,
      ...currentUser,
...
export const deleteEmployHis = createAsyncThunk("profile/employment/delete", async (formData, { getState, rejectWithValue }) => {
  try {
    const response = await deleteEmploymentHisApi(formData);
    const currentUser = getState().profile.user;
    const updatedEmpHisyory = currentUser.EmpolymentHistory.filter((emp) => emp.id !== formData?.employmentId);
    return {
      success: response.success,
      ...currentUser,
...
export const createServiceMentor = createAsyncThunk("create/service/mentor", async (formData, { getState, rejectWithValue }) => {
  try {
...
export const updateServiceApiMentor = createAsyncThunk("update/service/mentor", async ({ serviceId, formData }, { getState, rejectWithValue }) => {
...
export const deleteServiceMentor = createAsyncThunk("delete/service/mentor", async (serviceId, { getState, rejectWithValue }) => {
  try {
    await deleteMentorServiceApi(serviceId);
    const currentUser = getState().profile.user;

    const updatedServices = currentUser.services.filter((serv) => serv.id !== serviceId);
    const updatedUser = {
      ...currentUser,
      services: updatedServices,
      success: true,
...
export const updateOtherInfo = createAsyncThunk("profile/update/other/info", async (formData, { rejectWithValue }) => {
  try {
...
export const uploadMentorVideo = createAsyncThunk("profile/update/mentor/video", async (formData, { rejectWithValue }) => {
  try {
...
const profileSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateOtherInfo.pending, (state) => {
        state.error = null;
      })
      .addCase(updateOtherInfo.fulfilled, () => {})
      .addCase(updateOtherInfo.rejected, (state, action) => {
        state.error = action.payload;
      })
