  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (USER_ROLE === "JOB_SEEKER") {
        ...
        apiRoute = await employerProfileApi();
      } else {
  ...
  async (formData, { getState, rejectWithValue }) => {
    try {
      ...
      const updatedEducation = currentUser.Education.filter(
        (edu) => edu.id !== formData?.educationId
      );
  ...
  async (formData, { getState, rejectWithValue }) => {
    try {
      ...
      const updatedEmpHisyory = currentUser.EmpolymentHistory.filter(
        (emp) => emp.id !== formData?.employmentId
      );
      return {
        success: response.success,
        ...currentUser,
...
export const updateOtherInfo = createAsyncThunk(
  "profile/update/other/info",
