...
import {
  ...
  fetchRecruiterServices,
  updateRecruiterService,
} from "../../../features/admin/common-management/slices/recruiterServicesSlice";
...
const RecruiterServices = () => {
              ...
              },
              {
                ...
                onClick: () => {
                  Modal.confirm({
                    ...
                    onOk() {
                      try {
                        dispatch(deleteRecruiterService(record?.id));
                        message.open({
                          type: "success",
                          content: "Recruiter Service Deleted",
                        });
                      } catch (error) {
  ...
  const handleADD = async () => {
    try {
      ...
      await dispatch(addRecruiterService(values)).unwrap();
      handleClose();
      message.open({
        type: "success",
        content: "Recruiter Service Created",
      });
    } catch (error) {
      message.error(error.message || "Internal Server Error");
    }
  ...
  const handleUpdate = async () => {
    try {
      await form.validateFields();
      const values = form.getFieldsValue();
      await dispatch(updateRecruiterService({ id: industryId, data: values }))
        .unwrap;
      handleClose();
      message.open({
        type: "success",
        content: "Recruiter Service Updated",
      });
    } catch (error) {
      message.error(error.message || "Internal Server Error");
    }
      ...
      >
        ...
        >
          <Form size="large" form={form}>
            ...
            >
              <CommonInput placeholder="Enter service Name" />
            </Form.Item>
          </Form>
        </CommonModal>
      </Card>
